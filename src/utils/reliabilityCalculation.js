 ...
 * 
 * Facteurs inclus:
 * 1. Moyenne des notes: 30%
 * 2. Taux de missions terminées: 20%
 * 3. Rapidité d'acceptation: 10%
 * 4. Nombre de services actifs: 8%
 * 5. Complétude du profil: 8%
 * 6. Badges obtenus: 8%
 * 7. Paiements complétés: 7%
 * 8. Temps de réponse chat: 5%
 * 9. Taux d'annulation: -12%
 */
export async function calculateReliabilityScore(userId) {
  try {
    // 1️⃣ NOTES ET AVIS (30%)
    ...
    const ratingWeight = 0.30; // 30%

    // 2️⃣ TAUX DE MISSIONS TERMINÉES (20%)
    ...
    const completionRate = completionData.total > 0
      ...
      : 0;
    const completionWeight = 0.20; // 20%

    // 3️⃣ RAPIDITÉ D'ACCEPTATION BOOKING (10%)
    ...
    const acceptanceWeight = 0.10; // 10%

    // 4️⃣ NOMBRE DE SERVICES ACTIFS (8%)
    ...
    const servicesWeight = 0.08; // 8%

    // 5️⃣ COMPLÉTUDE DU PROFIL (8%)
    ...
    const profileWeight = 0.08; // 8%

    // 6️⃣ BADGES OBTENUS (8%)
    ...
    const badgesWeight = 0.08; // 8%

    // 7️⃣ PAIEMENTS COMPLÉTÉS (7%)
